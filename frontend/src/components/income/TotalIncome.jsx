...
const TransactionItem = ({ transaction, onEdit, onDelete, isEntryDeleting }) => {
  return (
    <div className="group relative flex flex-wrap md:flex-nowrap items-center gap-3 mt-4 p-3 rounded-lg hover:bg-base-100/60">
      {/* Transaction Icon */}
...
// Main component that handles all the logic once
function TotalIncome() {
  ...
  if (totalIncomeTransactions.length === 0) {
    return (
  ...
  // Split transactions into two columns
  const firstColumnTransactions = totalIncomeTransactions.slice(0, 5);
  const secondColumnTransactions = totalIncomeTransactions.slice(5, 10);
  const isLastBatch = totalIncomeTransactions.length <= 10;
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="rounded-lg">
          {firstColumnTransactions.map((transaction) => (
            <TransactionItem
              key={transaction._id}
        ...
        {/* Second Column */}
        <div className="rounded-lg">
          {secondColumnTransactions.map((transaction) => (
            <TransactionItem
              key={transaction._id}
              transaction={transaction}
              onEdit={handleOpenModal}
      ...
      {isLastBatch && totalIncomeTransactions.length > 0 && (
